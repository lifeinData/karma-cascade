...
1. **Matched gems explode with golden sparkles** (5 animated particles per gem) and disappear completely
2. **Remaining gems fall down smoothly** with physics-based animations to fill empty spaces  
3. **New gems appear at the top** and fall with staggered timing based on row position
4. **System checks for new matches** created by fallen or new gems using connected component detection
5. **Cascade continues automatically** until the board reaches a stable state with no matches
...
Built with modern React architecture, the game uses a clean component hierarchy (App → Game → Board → Gem → MatchEffects) with full TypeScript integration and a sophisticated centralized animation system. The responsive absolute positioning layout automatically adapts to any board size from 3x3 to 15x15, with smooth CSS animations and visual effects providing consistent gameplay across all devices.
...
- **Dazzling Match Effects**: Matched gems explode with 5 animated sparkle particles featuring golden radial gradients (#fff to #ffeb3b) before disappearing completely
- **Smooth Gravity Animations**: New gems fall from above with realistic cubic-bezier(0.25, 0.46, 0.45, 0.94) physics curves and staggered timing (100ms delay per row)
- **Centralized Animation System**: AnimationManager singleton coordinates match sparkles (0.5s duration), gravity falling (0.8s + stagger), and cascade timing with proper callback handling
- **Cascade Loop Prevention**: Advanced state management prevents infinite loops with isProcessingCascade flag and step-by-step processing
...
- **Component-Based Design**: Clean React hierarchy (App → Game → Board → Gem → MatchEffects) with proper separation of concerns
- **Advanced Animation System**: Centralized AnimationManager singleton coordinates gem falling, sparkle effects, and cascade timing with callback management
- **Responsive Absolute Positioning**: Dynamic layout with absolute positioning for smooth animations and touch-optimized 40x40px gems
...
- **Modular Styling**: Separate CSS files (animations.css, matchEffects.css, index.css) for core animations, match effects, and responsive design with Tailwind CSS integration
- **Mobile-First Design**: HTML viewport configuration with user-scalable=no for touch controls and full-height responsive layout
...
- **Advanced Step-by-Step Gravity Physics Engine**: Coordinated cascade system (Remove → Gravity → Fill → Check → Repeat → Stabilize) with complete gem disappearance and realistic falling mechanics
...
- **Centralized Animation Management**: AnimationManager singleton coordinates all visual effects, timing, and callbacks for seamless user experience with proper cascade loop prevention
- **Advanced Visual Effects System**: 5 golden sparkle particles per gem with radial gradients (#fff to #ffeb3b), staggered timing (0-150ms delays), and burst animations for matched gems
...
- **CSS Keyframe Integration**: Sophisticated animation system with gravity-fall, gem-disappear-with-sparkles, sparkle-burst, and cascade delay keyframes
...
- **Gravity Physics**: Complete step-by-step gravity cascade system with gem removal, falling mechanics, top refill, and infinite cascade loops until board stabilization with proper loop prevention
- **Visual Effects**: Fully implemented 5-particle sparkle system with golden burst animations, smooth gem falling with physics curves, and coordinated timing via AnimationManager
- **Animation System**: Centralized AnimationManager singleton handles all visual effects, match sparkles (0.5s), gravity animations (0.8s + stagger), and cascade coordination with callback management
...
- **Cross-Platform**: Responsive design works perfectly on desktop and mobile with touch-optimized controls (user-scalable=no) and smooth animations
...
- **TypeScript Integration**: Full type safety with comprehensive interfaces for all game logic, step-by-step gravity system, animation states, and component interactions
...
**Component Structure:**
- **App.tsx**: Main application wrapper with Tailwind CSS flexbox layout and full-height responsive design
- **Game.tsx**: Core game logic, state management, step-by-step cascade coordination, and live level generator controls with comprehensive logging
- **Board.tsx**: Dynamic absolute positioning layout that adapts to any board size (3x3 to 15x15) with MatchEffects overlay and responsive styling
- **Gem.tsx**: Individual gem components with click handling, hover effects, selection feedback, and AnimationManager integration
- **MatchEffects.tsx**: Sparkle particle system that renders 5 golden burst animations per matched gem with radial gradients
...
- **matchDetection.ts**: Unified connected component detection system for both schema validation and gameplay matching
...
- **cleanup.ts**: Gray gem cleanup system with retry logic and fallback to regeneration
...
- **AnimationManager.ts**: Centralized animation coordinator managing match effects, gravity timing, callback handling, and visual state coordination
- **Game.tsx**: Step-by-step cascade processing (initial/gravity/newGems) with proper loop prevention and state management

**Animation & Effects System:**
- **AnimationManager.ts**: Singleton managing match sparkles (0.5s), gravity falling (0.8s + stagger), and cascade timing with callback coordination
- **MatchEffects.tsx**: React component rendering 5 sparkle particles per gem with golden radial gradients and staggered burst timing
- **animations.css**: CSS keyframes for gravity-fall (cubic-bezier physics), gem-appear, gem-land-bounce, and cascade delays (0.1s-0.5s)
- **matchEffects.css**: Sparkle particle animations with burst patterns, radial gradients (#fff to #ffeb3b), and gem-disappear-with-sparkles effects

**Entry Point & Styling:**
- **main.tsx**: React application bootstrap with StrictMode, imports all CSS files (index.css, animations.css, matchEffects.css)
- **index.html**: Mobile-optimized HTML structure with responsive viewport, user-scalable=no for touch controls, and full-height layout
- **index.css**: Tailwind CSS integration plus custom game styling (40px gems, hover effects, selection highlighting)
- **constants.ts**: Game configuration constants including GEM_SIZE_PX = 40 for consistent sizing
...
- **Valid Move**: Gems swap colors instantly, triggering the revolutionary cascade system:
  1. **Matched gems explode with golden sparkles** - 5 animated sparkle particles (4-10px sizes) burst from each matched gem with radial gradients (#fff to #ffeb3b)
  2. **Matched gems disappear completely** after 0.5s sparkle animation using gem-disappear-with-sparkles keyframes (no placeholders!)
  3. **Remaining gems fall down smoothly** with cubic-bezier(0.25, 0.46, 0.45, 0.94) physics curves, filling empty spaces naturally
  4. **New gems appear at the top** and fall with staggered timing (100ms delay per row) using gravity-fall animation
  5. **System checks for new matches** created by fallen or new gems using connected component detection
  6. **Cascade continues step-by-step** with coordinated AnimationManager callbacks until the board reaches a stable state with no matches
